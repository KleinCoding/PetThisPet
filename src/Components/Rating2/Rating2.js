...
class Rating2 extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      rating: 0,
      preciseValue:
        this.props.initialValue > this.props.numberStars
          ? this.props.initialValue % this.props.numberStars
  ...
  handleClick(index) {
    ...
    if (this.props.callback) this.props.callback(index + 1)
  ...
  calculateWidth(i) {
    ...
    if (i + 1 <= value || i - 1 < this.props.initialValue) return 100 + "%";
    // Full star
    else if (i < value) return (value % 1) * 100 + "%"; // Portion star

    return 0; // Empty
  }

  componentDidUpdate() {
    console.log("rating.js did update");
  }
  render() {
    const divStyle = {
      width: this.width,
    ...
    if (this.state.editable) {
      for (let i = 0; i < numberStars; i++) {
        ...
        stars[i] = (
          <EditableStars
            key={i}
            image="http://127.0.0.1:8080/star2.png"
      ...
      return (
        <div
          ...
          onMouseEnter={this.handleContainerEnter.bind(this)}
          onMouseLeave={this.handleContainerLeave.bind(this)}
        >
          {stars}
        </div>
      );
      ...
      for (let i = 0; i < numberStars; i++) {
        ...
        stars[i] = (
          <NonEditableStars
            key={i}
            image="http://127.0.0.1:8080/heart.png"
            style={style}
            divStyle={divStyle}
            initialBG={this.props.initialBG}
          />
...
const EditableStars = props => {
  return (
    <div className="divStar" style={props.divStyle}>
      ...
      <div
        className="fillInitialBG"
        style={{ background: props.initialBG }}
      ></div>
      <img
        src={props.image}
        alt="Rating Icon"
  ...
  return (
    <div className="divStar" style={props.divStyle}>
      ...
      <div
        ...
        style={{ background: props.initialBG }}
      ></div>
      <img src={props.image} alt="Rating Icon" />
    </div>
  );
};
